...
/// can answer (no device, no CUDA libraries, ...).
///
/// The **runtime** API is used, not the driver's `cuMemGetInfo_v2`.  The driver
/// call needs a context that is already current: outside one it fails with
/// `CUDA_ERROR_NOT_INITIALIZED` (3), and with only `cuInit` called it fails
/// with `CUDA_ERROR_NOT_READY` (201) and reports zero bytes.  `cudaMemGetInfo`
/// initialises the runtime itself and answers correctly either way, which
/// matters because this runs *before* the engine's context exists.
fn device_memory() -> Option<(u64, u64, String)> {
    let cudart = Lib::open(&["libcudart.so.12", "libcudart.so"]).ok()?;
    // SAFETY: documented signature of cudaMemGetInfo.
    let (free, total) = unsafe {
        let mem_info: unsafe extern "C" fn(*mut usize, *mut usize) -> i32 =
            cudart.sym("cudaMemGetInfo").ok()?;
        let (mut free, mut total) = (0usize, 0usize);
        if mem_info(&mut free, &mut total) != 0 {
            return None;
        }
        (free as u64, total as u64)
    };
    let name = Lib::open(&["libcuda.so.1", "libcuda.so"])
        .ok()
        .map(|d| device_name(&d))
        .unwrap_or_else(|| "the GPU".to_string());
    Some((free, total, name))
}

/// The current device's name, or a placeholder.  Every query is optional, in
/// the same spirit as `device_note`.
fn device_name(driver: &Lib) -> String {
    // SAFETY: documented signatures.
    unsafe {
        let device_get: Result<unsafe extern "C" fn(*mut i32, i32) -> i32, String> =
            driver.sym("cuDeviceGet");
        let name_of: Result<
            unsafe extern "C" fn(*mut std::ffi::c_char, i32, i32) -> i32,
            String,
        > = driver.sym("cuDeviceGetName");
        let (device_get, name_of) = match (device_get, name_of) {
            (Ok(a), Ok(b)) => (a, b),
            _ => return "the GPU".to_string(),
        };
        let mut dev = 0i32;
        if device_get(&mut dev, 0) != 0 {
            return "the GPU".to_string();
        }
        let mut buf = [0 as std::ffi::c_char; 128];
        if name_of(buf.as_mut_ptr(), buf.len() as i32, dev) == 0 {
...
/// image on a small card reaches `cudaMalloc: out of memory` after several
...
pub fn check_capacity(h: usize, w: usize, hint: &str) -> Result<(), String> {
    ...
    let (free, total, name) = match device_memory() {
        ...
        // so, because a guard that silently never fires is worse than none (this
        // one did exactly that while it used the driver's memory query).
        None => {
            eprintln!(
...
/// Bytes as a human string, in GiB or MiB.
