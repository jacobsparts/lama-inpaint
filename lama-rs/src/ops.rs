...
//!
//! The GPU path runs convolutions as cuBLAS GEMMs over an im2col patch matrix,
//! and the Fourier units through cuFFT; both libraries are loaded at runtime
//! with `dlopen` (see `cuda.rs`) so the binary builds without CUDA headers or
//! nvcc.
